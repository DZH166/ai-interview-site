...
(async () => {
  ...
  try {
    ...
    check('3752 questions are loaded and new questions have no fabricated practice', await page.evaluate(() => Data.allQuestions().length === 3752 && Data.allQuestions().filter(q => q.topic === 'langchain').every(q => !Store.rec(q.id).practiceCount && !Store.rec(q.id).status)));
